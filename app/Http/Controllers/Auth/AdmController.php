...
{
    public $acceptedFormats = [ 'gif' , 'png' , 'PNG' ,'jpg' , 'JPG' , 'pdf' , 'bmp' , 'svg' , 'txt' , 'xls' , 'dbf' , 'ico' , 'mp4' ];
    ...
    {
        $data = [
            "view"      => "auth.parts.elim",
            "title"     => "Papelera",
            "imagenes"  => Imagen::where( "elim" , 1 )->get(),
            "sliders"  => Slider::where( "elim" , 1 )->get(),
            "buttons" => [
    ...
    public function imagen( Request $request )
    {
        ...
        $data = [
            ...
            "title"     => "Imágenes",
            "elementos"  => Imagen::whereNull( "elim" )->orWhere( "elim" , 0 )->get(),
            "buttons" => [
                [ "i" => "fas fa-trash-alt" , "b" => "btn-danger" , "t" => "Eliminar" ]
            ],
        ];
    ...
    public function imagenStore(Request $request, $data = null) {
        self::store( new Imagen , $request , $data );
    }
    public function imagenDestroy( Request $request )
    {
        self::destroy( new Imagen , $request );
    ...
    public function destroy( $model , $request )
    {
        DB::beginTransaction();
        try {
            self::delete( $model->find( $request->all()[ "id" ] ) , $model->getFillable() );
            DB::commit();
            echo 1;
        } catch (\Throwable $th) {
            DB::rollback();
