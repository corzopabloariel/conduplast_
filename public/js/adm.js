/**
 * @date 01.2020
 * @last_change 22.01.2020
 * @version 0.3.2.1
 **/
const colorPick = "C82828,7B7B7B,1B6592,3FC2F1,505050,C8C8C8,3E3F44,444444,FFFFFF,7E90A1,666666";
...
remove_ = ( t , class_ ) => {
    ...
    Swal.fire({
        ...
        if ( result.value ) {
            ...
            if( target.find( ".remove-element" ).length ) {
                target.find( ".remove-element" ).val( 1 );
 ...
 *      GUARDAR ELEMENTO
 ** ------------------------------------- */
formSave = ( t , formData , message = { wait : "Espere. Guardando contenido" , err: "Ocurrió un error en el guardado. Reintente" , catch: "Ocurrió un error en el guardado." , success : "Contenido guardado" , ext : "Extensión no válida" } , callback = null ) => {
    let url = t.action;
    let method = t.method;
    if( !verificarForm() )
        return null;
    ...
    .then((res) => {
        if( callback === null ) {
            if(parseInt(res.data) == 1) {
                ...
                Swal.fire(
                    'Extensión no válida',
                    `Extensiones permitidas: 'gif' , 'png' ,'jpg' , 'pdf' , 'bmp' , 'svg' , 'txt' , 'xls' y 'dbf'`,
                    'error'
                )
            } else  {
                $( "body > .wrapper" ).removeClass( "isDisabled" );
...
formSubmit = ( t ) => {
    ...
    formData.append("ATRIBUTOS", JSON.stringify(window.formArr) );
    if( verificarForm() ) {
...
shortcut.add( "Alt+Ctrl+E" , function () {
    window.location = `${url_simple}/adm/elim`;
}, {
    type: "keydown",
    propagate: true,
    target: document
 ...
 ** ------------------------------------- */
openElement = (t) => {
    if (t.firstChild.classList.contains("fa-eye")) {
        t.firstChild.classList.remove("fa-eye");
        t.firstChild.classList.add("fa-eye-slash");
    } else {
        t.firstChild.classList.remove("fa-eye-slash");
        t.firstChild.classList.add("fa-eye");
    }
    let element = t.nextSibling.nextSibling;
...
init = ( callbackOK , table = true , type = "table" , without = false ) => {
    /** */
    if (window.pyrus !== undefined ) {
        window.formArr = [{ DATA: window.pyrus.objetoSimple, TIPO: "U" }];
