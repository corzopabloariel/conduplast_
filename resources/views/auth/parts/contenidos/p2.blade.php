<section class="my-3 wrapper-home">
    <div class="container-fluid">
        ...
        <div class="mb-4" style="display: none;">
            @include( 'page.x_p2' , [ 'elementos' => $data[ 'elementos' ]->data ] )
        </div>
...
@push('scripts')
<script>
    window.pyrus_elemento_1 = new Pyrus( "contenido_p2_elemento_1" );
    window.pyrus_elemento_2 = new Pyrus( "contenido_p2_elemento_2" );

    addNumber = ( t , value = null ) => {
    ...
    init( () => {
        ...
        h += `<div class="row justify-content-center pt-3">`;
            h += `<div class="col-12">`;
                h += `<button type="button" class="btn px-4 btn-dark text-center text-uppercase" onclick="addNumber( this )">Elemento (Imagen + texto)<i class="fas fa-plus ml-2"></i></button>`;
            h += `</div>`;
        h += `</div>`;
        ...
        if( window.data.elementos.data !== null ) {
            ...
            window.data.elementos.data.e2.forEach( x => addNumber( null , x ) );
        }
    } );
</script>
@endpush
